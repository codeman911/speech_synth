#!/usr/bin/env python3
"""
Higgs Audio v2 Training Script for Zero-Shot Voice Cloning with ChatML Dataset
...
datasets in ChatML format. The implementation follows the same data processing pipeline as the 
inference script to ensure consistency between training and inference.
"""
...
try:
    ...
    # Add fallback class definitions
...
class ExtendedHiggsAudioSampleCollator:
    ...
    def __call__(self, batch: List[ChatMLDatasetSample]):
        if HIGGS_AVAILABLE and hasattr(self, 'base_collator'):
            # 1. Call the official, underlying collator to complete all complex padding and alignment work
            batch_input = self.base_collator(batch)
            
            # batch_input.audio_out_ids is padded and processed, its length is exactly consistent with the model output audio_logits length.
            label_audio_ids = batch_input.audio_out_ids
            
            # 2. Convert to our extended class and pass in this perfect label
            extended_batch = ExtendedHiggsAudioBatchInput(
                ...
                label_audio_ids=label_audio_ids, # <-- Use our newly defined, aligned labels
...
class HiggsAudioModelWrapper(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # CRITICAL: Force enable Whisper embeddings for better voice cloning (override model config)
        # This follows the training pipeline pattern where Whisper is always enabled
        if HIGGS_AVAILABLE and hasattr(self, 'config'):
            original_whisper_setting = getattr(self.config, 'encode_whisper_embed', False)
...
class HiggsAudioTrainer(Trainer):
    ...
    def __init__(self, **kwargs):
        ...
        # Add this line to get config from model
        self.config = self.model.config
    
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """Custom loss computation following the same pattern as trainer.py"""
...
def setup_lora_config(model: nn.Module, lora_config: Dict) -> nn.Module:
    """Setup LoRA configuration for the model following the same pattern as trainer.py"""
...
def main():
    parser = argparse.ArgumentParser(description="Train Higgs Audio v2 for Zero-Shot Voice Cloning")
    ...
    # Load datasets
    train_dataset = ZeroShotVoiceCloningDataset(
    ...
    if args.eval_data_file:
        eval_dataset = ZeroShotVoiceCloningDataset(
            args.eval_data_file,
            tokenizer,
            audio_tokenizer,
        )
    ...
    training_args = TrainingArguments(
        ...
        ddp_find_unused_parameters=True,  # For DDP compatibility
    )
    ...
    # Start training
    logger.info("Starting zero-shot voice cloning training")
    trainer.train()
